      ...
      fluidRow(
        column(width = 12,align="center",
               wellPanel(h1("What Will You Watch Next?",align="center"),
                         ...
                         submitButton("Search", width= 160), style = "display: block; text-align: center;")),
        column(11,align="left",
               conditionalPanel(
                 condition = "input.movieSeries == 'Movies'",
                 checkboxGroupInput("MGenres", "Movies Genre Selection",
                                    choices = c("Comedy", "Drama", "Science Fiction", 
                                                "Action","Animation", "Thriller","Horror",
                                                "Mystery","Adventure")),
...
server <- function(input, output) {
  ...
  # to filter those out
  output$tester <- renderText(if (length(input$movieSeries >= 0)) {
    if (input$movieSeries == "Series"){
      tv_data %>%
        filter(episode_run_time <= input$time[2] & episode_run_time > input$time[1]
               ...
               & adult == FALSE, genres.name %in% input$SGenres, 
               original_language %in% input$SLanguage) %>% arrange(episode_run_time) %>%
        ...
        mutate(name = paste0('<table><tr><td align=center><h2>',name,
                             ...
                             " Points ",'<br><a href="',ext_link,
                             '">Where to Watch</a></td></tr></table><br>')) %>%
        pull(name)
